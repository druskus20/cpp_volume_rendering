...
void RenderingManager::SetImGuiInterface()
{
    ...
    if (ImGui::CollapsingHeader("Evaluation###EvaluationHeader"))
    {
      ...
      if (ImGui::Button("Start Evaluation"))
      {
        ...
        if (m_eval_csvfile.is_open())
          m_eval_csvfile.close(); // Should really not happen, but better be save.
  ...
  if (m_imgui_data_window)
  {
    ...
    if (ImGui::CollapsingHeader("Input Volume###DataManagerInputVolume"))
    {
      ...
      if (m_data_mgr.GetInputVolumeDataType() == vis::GRID_VOLUME_DATA_TYPE::STRUCTURED)
      {
        ...
        if (m_data_mgr.GetCurrentStructuredVolume() != nullptr)
        {
          ...
          ImGui::BulletText("Resolution: %d %d %d", m_data_mgr.GetCurrentStructuredVolume()->GetWidth(), m_data_mgr.GetCurrentStructuredVolume()->GetHeight(), m_data_mgr.GetCurrentStructuredVolume()->GetDepth());
          ImGui::BulletText("Voxel Size: %.2f %.2f %.2f", m_data_mgr.GetCurrentStructuredVolume()->GetScaleX(), m_data_mgr.GetCurrentStructuredVolume()->GetScaleY(), m_data_mgr.GetCurrentStructuredVolume()->GetScaleZ());
        }
    ...
    if (ImGui::Button("Load Importances"))
    {
      int width = 256;
      int height = 256;
      int depth = 178;

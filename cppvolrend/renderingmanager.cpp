...
bool RenderingManager::Exists() { return (crr_instance != NULL); }
...
void RenderingManager::InitData() {
  ...
  m_camera_state_list.ReadCameraStates(path_data_folder1 +
                                       "#list_camera_states");
  m_std_cam_state_names.clear();
  for (int i = 0; i < m_camera_state_list.NumberOfCameraStates(); i++)
    m_std_cam_state_names.push_back(
        m_camera_state_list.GetCameraState(i)->cam_setup_name);
  m_current_camera_state_id = 0;
  curr_rdr_parameters.GetCamera()->SetData(
      m_camera_state_list.GetCameraState(m_current_camera_state_id));

  // Read light source lists
  std::string path_data_folder2(MAKE_STR(CMAKE_PATH_TO_DATA_FOLDER));
  m_light_source_list.ReadLightSourceLists(path_data_folder2 +
                                           "#list_light_sources");
  ...
  m_current_lightsource_data_id =
      glm::clamp(m_current_lightsource_data_id, 0,
                 m_light_source_list.NumberOfLists() - 1);

  curr_rdr_parameters.EraseAllLightSources();
  for (int i = 0;
       i < m_light_source_list.GetList(m_current_lightsource_data_id)
               ->m_lightsources.size();
       i++)
    curr_rdr_parameters.CreateNewLightSource(
        m_light_source_list.GetList(i)->m_lightsources[i]);
  ...
  for (int i = 0; i < m_vtr_vr_methods.size(); i++)
    m_vtr_vr_methods[i]->SetExternalResources(&m_data_mgr,
                                              &curr_rdr_parameters);
  ...
  // Reshape
  Reshape(curr_rdr_parameters.GetScreenWidth(),
          curr_rdr_parameters.GetScreenHeight());
}
...
void RenderingManager::Display() {
  ...
  if (m_eval_running) {
    ...
    // The frame rate display (console and ImGui) is only updated when not
    // evaluating. Evaluation computes the frame rate after a given fixed number
    // of frames.
    UpdateFrameRate();
  }
  ...
  if (animate_camera_rotation) {
    ...
    glm::vec3 cright =
        glm::normalize(glm::cross(curr_rdr_parameters.GetCamera()->GetDir(),
  ...
  if (m_eval_running) {
    ...
    if (m_eval_currframe >= m_eval_numframes) {
      ...
      const double time_per_frame =
          (currenttime - m_eval_lasttime) / m_eval_numframes;
      ...
      size_t n_zero = 4;
      imagefilename =
          std::string(n_zero - std::min(n_zero, imagefilename.length()), '0') +
          imagefilename + ".png";
...
void RenderingManager::CloseFunc() {
  ...
  for (int i = m_vtr_vr_methods.size() - 1; i >= 0; i--)
    delete m_vtr_vr_methods[i];
...
void RenderingManager::UpdateDataAndResetCurrentVRMode() {
  curr_vol_renderer->Init(curr_rdr_parameters.GetScreenWidth(),
                          curr_rdr_parameters.GetScreenHeight());
}
...
void RenderingManager::SaveScreenshot(std::string filename) {
  ...
  if (filename.empty()) {
    filename =
        AddAbreviationName(curr_rdr_parameters.GetDefaultScreenshotName());
  }
  GenerateImgFile(filename, curr_rdr_parameters.GetScreenWidth(),
                  curr_rdr_parameters.GetScreenHeight(), rgb_data, false);
  delete[] rgb_data;
}
...
void RenderingManager::UpdateLightSourceCameraVectors() {
  ...
  curr_rdr_parameters.SetBlinnPhongLightSourceCameraVectors(cforward, cup,
                                                            cright);
}
...
std::string RenderingManager::AddAbreviationName(std::string filename,
  ...
  out_str =
      out_str + "_" + curr_vol_renderer->GetAbbreviationName() + extension;

  return out_str;
}

bool RenderingManager::GenerateImgFile(std::string out_str, int w, int h,
                                       unsigned char *gl_data, bool alpha,
  ...
  int user_color_mode =
      alpha ? IM_RGB | IM_ALPHA | IM_PACKED : IM_RGB | IM_PACKED;
...
GLubyte *RenderingManager::GetFrontBufferPixelData(bool alpha) {
  GLubyte *gl_img_data =
      new GLubyte[(alpha ? 4 : 3) * curr_rdr_parameters.GetScreenWidth() *
                  curr_rdr_parameters.GetScreenHeight()];
  ...
  glReadPixels(0, 0, curr_rdr_parameters.GetScreenWidth(),
               curr_rdr_parameters.GetScreenHeight(),
               (alpha ? GL_RGBA : GL_RGB), GL_UNSIGNED_BYTE, gl_img_data);
...
void RenderingManager::SetCurrentVolumeRenderer() {
  if (curr_vol_renderer)
    curr_vol_renderer->Clean();

  curr_vol_renderer = m_vtr_vr_methods[m_current_vr_method_id];
  if (curr_vol_renderer->GetDataTypeSupport() ==
      m_data_mgr.GetInputVolumeDataType()) {
...
void RenderingManager::SetImGuiInterface() {
  ...
  if (m_imgui_render_manager) {
    ...
    if (ImGui::Button("Generate Diff###BTNgeneratediffimage")) {
      ...
      // tf1d.AddRGBControlPoint(vis::TransferControlPoint(1.0, 0.0, 0.0, (0.10
      // * 255.0))); tf1d.AddRGBControlPoint(vis::TransferControlPoint(0.0, 1.0,
      // 0.0, (0.55 * 255.0)));
      // tf1d.AddRGBControlPoint(vis::TransferControlPoint(0.0, 0.0, 1.0, (1.00
      // * 255.0)));

      tf1d.AddRGBControlPoint(vis::TransferControlPoint(1.0, 1.0, 1.0, 0.0));
      tf1d.AddRGBControlPoint(
          vis::TransferControlPoint(1.0, 0.0, 0.0, (0.40 * 255.0)));
      tf1d.AddRGBControlPoint(
          vis::TransferControlPoint(1.0, 0.0, 0.0, (1.00 * 255.0)));
      tf1d.Build();
      ...
      // if (s_tex_diff_ref_screen)
      {
        ...
        for (int i = 0; i < o_w * o_h; i++) {
          ...
          // double c_alpha_r = glm::clamp((Cie2000Comparison(rgb_ref, rgb_cur)
          // / 100.0) / s_diff_max_c_alpha, 0.0, 1.0);
    ...
    ImGui::Separator();
    if (ImGui::Button(
            "Export LAB Diff Transfer Function Image###BTNexportimagetfdiff")) {
...

      {
        ...
        GenerateImgFile(out_tf_str, img_w, img_h, tf_rgb_img, false);
      }
    ...
    if (ImGui::Button("Export Transfer Function Image###BTNexportimagetf")) {
      ...
      for (int i = 0; i < img_w; i++) {
        glm::vec4 clr = m_data_mgr.GetCurrentTransferFunction()->Get(
            (double(i) / double(img_w)) * 255.0);
        for (int j = 0; j < img_h; j++) {
          int idx = (i + (j * img_w)) * 3;
          tf_rgb_img[idx + 0] = clr.r * 255.0;
          tf_rgb_img[idx + 1] = clr.g * 255.0;
    ...
    ImGui::Text("- Viewport:");
    int v_size[2] = {curr_rdr_parameters.GetScreenWidth(),
                     curr_rdr_parameters.GetScreenHeight()};
    if (ImGui::DragInt2("###WINDOWVIEWPORT", v_size, 1,
                        RENDERING_MANAGER_MIN_VIEWPORT_SIZE,
                        RENDERING_MANAGER_MAX_VIEWPORT_SIZE)) {
#ifdef USING_FREEGLUT
    ...
    if (ImGui::CollapsingHeader("Evaluation###EvaluationHeader")) {
      ...
      ImGui::Text("%d parameters with %d total samples",
                  m_eval_paramspace.GetNumDimensions(), numsamples);
      ...
      const double neededtime =
          ceil(m_ts_window_ms * numsamples * m_eval_numframes / 1000.);
      ImGui::Text("approx. %.0f seconds for evaluation at current FPS",
                  neededtime);
      if (ImGui::Button("Start Evaluation")) {
        // Name of a directory containing all the evaluation files - naming is
        // datetime-based
        ...
        if (m_eval_csvfile.is_open())
          m_eval_csvfile
              .close(); // Should really not happen, but better be save.
        m_eval_csvfile.open(m_eval_basedirectory + "/eval.csv",
                            std::ios_base::out);
        ...
        if (m_eval_csvfile.is_open()) {
          ...
          // On the other hand, we want to measure the speed of the volume
          // renderer and not of ImGui.
    ...
    if (ImGui::CollapsingHeader("Camera###CameraSettingsHeader")) {
      ...
      if (ImGui::Combo("###ImArrayLoadCurrentCameraState",
                       &m_current_camera_state_id, vector_getter,
                       static_cast<void *>(&m_std_cam_state_names),
                       m_std_cam_state_names.size())) {
        curr_rdr_parameters.GetCamera()->SetData(
            m_camera_state_list.GetCameraState(m_current_camera_state_id));
        curr_vol_renderer->SetOutdated();
      }
    ...
    if (ImGui::CollapsingHeader("Light Sources###LightSourceManagerHeader")) {
      ...
      if (ImGui::Combo("###CurrentLightSourcePositionData",
                       &m_current_lightsource_data_id, vector_getter,
                       static_cast<void *>(&m_std_lsource_names),
                       m_std_lsource_names.size())) {
        ...
        for (int i = 0;
          ...
          lsd.position =
              m_light_source_list.GetList(m_current_lightsource_data_id)
                  ->m_lightsources[i]
                  .position;
          lsd.z_axis =
              m_light_source_list.GetList(m_current_lightsource_data_id)
      ...
      for (int i = 0; i < curr_rdr_parameters.GetNumberOfLightSources(); i++) {
...

        {
          ...
          if (ImGui::DragFloat(lsp_v1.c_str(), &lsd->position.x, 0.5f, -FLT_MAX,
                               FLT_MAX, "%.1f")) {
            curr_vol_renderer->SetOutdated();
          }
          ...
          if (ImGui::DragFloat(lsp_v2.c_str(), &lsd->position.y, 0.5f, -FLT_MAX,
                               FLT_MAX, "%.1f")) {
            curr_vol_renderer->SetOutdated();
          }
          ...
          if (ImGui::DragFloat(lsp_v3.c_str(), &lsd->position.z, 0.5f, -FLT_MAX,
                               FLT_MAX, "%.1f")) {
            curr_vol_renderer->SetOutdated();
          }
        }
        ...
        if (ImGui::DragFloat(lsp_ed.c_str(), &lsd->energy_density, 0.1f, 0.0f,
                             FLT_MAX, "%.1f")) {
          curr_vol_renderer->SetOutdated();
        }

        {
          ...
          if (ImGui::DragFloat(lsp_clr_v1.c_str(), &lsd->color.x, 0.01f, 0.0f,
                               1.0f, "%.2f")) {
            curr_vol_renderer->SetOutdated();
          }
          ...
          if (ImGui::DragFloat(lsp_clr_v2.c_str(), &lsd->color.y, 0.01f, 0.0f,
                               1.0f, "%.2f")) {
            curr_vol_renderer->SetOutdated();
          }
          ...
          if (ImGui::DragFloat(lsp_clr_v3.c_str(), &lsd->color.z, 0.01f, 0.0f,
                               1.0f, "%.2f")) {
            curr_vol_renderer->SetOutdated();
          }
        }
        ...
        if (ImGui::Combo(lsp_ltype.c_str(), &ilt, items_rendermode,
                         IM_ARRAYSIZE(items_rendermode))) {
  ...
  if (m_imgui_data_window) {
    ...
    if (ImGui::CollapsingHeader("Input Volume###DataManagerInputVolume")) {
      ...
      if (m_data_mgr.GetInputVolumeDataType() ==
          vis::GRID_VOLUME_DATA_TYPE::STRUCTURED) {
        if (m_data_mgr.GetNumberOfStructuredDatasets() > 0) {
          int volume_index = m_data_mgr.GetCurrentVolumeIndex();
          if (ImGui::Combo(
                  "###CurrentVolumeDataID", &volume_index, vector_getter,
                  static_cast<void *>(m_data_mgr.GetUINameDatasetListPtr()),
                  m_data_mgr.GetUINameDatasetListPtr()->size())) {
        ...
        if (m_data_mgr.GetCurrentStructuredVolume() != nullptr) {
          ...
          ImGui::BulletText(
              "Resolution: %d %d %d",
              m_data_mgr.GetCurrentStructuredVolume()->GetWidth(),
              m_data_mgr.GetCurrentStructuredVolume()->GetHeight(),
              m_data_mgr.GetCurrentStructuredVolume()->GetDepth());
          ImGui::BulletText(
              "Voxel Size: %.2f %.2f %.2f",
              m_data_mgr.GetCurrentStructuredVolume()->GetScaleX(),
              m_data_mgr.GetCurrentStructuredVolume()->GetScaleY(),
              m_data_mgr.GetCurrentStructuredVolume()->GetScaleZ());
        }

        if (ImGui::CollapsingHeader(
                "Gradient Volume###DataManagerGradientVolume")) {
          int gradient_gen_index =
              m_data_mgr.GetCurrentGradientGenerationTypeID();
          std::vector<std::string> g_list =
              m_data_mgr.GetGradientGenerationTypeStrList();
          if (ImGui::Combo("###CurrentGradientGenerationDataID",
                           &gradient_gen_index, vector_getter,
                           static_cast<void *>(&g_list), g_list.size())) {
            if (gradient_gen_index !=
                m_data_mgr.GetCurrentGradientGenerationTypeID()) {
    ...
    ImGui::Separator();
    if (ImGui::CollapsingHeader(
            "Transfer Function###DataMdanagerTransferFunction")) {
      int transfer_function_index =
          m_data_mgr.GetCurrentTransferFunctionIndex();
      if (ImGui::Combo("###CurrentTransferFunction1DID",
                       &transfer_function_index, vector_getter,
                       static_cast<void *>(
                           m_data_mgr.GetUINameTransferFunctionListPtr()),
                       m_data_mgr.GetUINameTransferFunctionListPtr()->size())) {
        if (transfer_function_index !=
            m_data_mgr.GetCurrentTransferFunctionIndex()) {
  ...
  if (m_imgui_renderer_window) {
    ...
    if (ImGui::Combo("###CurrentBaseVolumeRenderer", &m_current_vr_method_id,
                     vector_getter, static_cast<void *>(&m_vtr_vr_ui_names),
                     m_vtr_vr_ui_names.size())) {
      ResetGLStateConfig();
      SetCurrentVolumeRenderer();
    }
...
void RenderingManager::UpdateFrameRate() {
  ...
  if ((m_ts_current_time - m_ts_last_time) >
      RENDERING_MANAGER_TIME_PER_FPS_COUNT_MS) {
    m_ts_window_fps =
        double(m_ts_n_frames) * 1000.0 / (m_ts_current_time - m_ts_last_time);
...
RenderingManager::RenderingManager()
    : m_current_vr_method_id(0), curr_vol_renderer(NULL) {
...
RenderingManager::~RenderingManager() { CloseFunc(); }

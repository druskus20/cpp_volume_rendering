...
PvmOld::PvmOld (const char *file_name)
{
...
float* PvmOld::GetData ()
{
  return pvm_data;
}

float* PvmOld::GenerateNormalizeData ()
{
...
float* PvmOld::GenerateReescaledMinMaxData (bool normalized, float* fmin, float* fmax)
{
  float max_density_value = pow(2, components * 8) - 1;
...
void PvmOld::GetScale (double* sx, double* sy, double* sz)
{
  *sx = scalex;
  *sy = scaley;
  *sz = scalez;
...
void PvmOld::GetDimensions (unsigned int* _width, unsigned int* _height, unsigned int* _depth)
{
  *_width  = width;
  *_height = height;
  *_depth  = depth;
...
int PvmOld::GetComponents ()
{
  return components;
}

void PvmOld::TransformData (unsigned int dst_bytes_per_voxel)
{
...
float* PvmOld::PostProcessData (unsigned char* data)
{
...
unsigned char* DDSV3Old::readPVMvolume (const char* filename,
                                     ...
                                     unsigned char** comment)
{
  ...
  if ((data = (unsigned char*)realloc(data, bytes + 1)) == NULL) DDSOLD_ERRORMSG();
  data[bytes] = '\0';
  ...
  else if (numc != 1) DDSOLD_ERRORMSG();
  ...
  if ((volume = (unsigned char*)malloc((*width)*(*height)*(*depth)*numc + len1 + len2 + len3 + len4)) == NULL) DDSOLD_ERRORMSG();
  if (data + bytes != ptr + (*width)*(*height)*(*depth)*numc + len1 + len2 + len3 + len4) DDSOLD_ERRORMSG();
...
unsigned char* DDSV3Old::readPNMimage (const char* filename,
                                    unsigned int* width,
                                    unsigned int* height,
                                    unsigned int* components)
{
  ...
  {
    while (*ptr1 == '\n')
      if (++ptr1 >= data + bytes) DDSOLD_ERRORMSG();
    while (*ptr1 == '#')
      if (++ptr1 >= data + bytes) DDSOLD_ERRORMSG();
      else
        while (*ptr1 != '\n')
  ...
  ptr2 = ptr1;
  while (*ptr2 != '\n' && *ptr2 != ' ')
    if (++ptr2 >= data + bytes) DDSOLD_ERRORMSG();
  if (++ptr2 >= data + bytes) DDSOLD_ERRORMSG();
  while (*ptr2 != '\n' && *ptr2 != ' ')
    if (++ptr2 >= data + bytes) DDSOLD_ERRORMSG();
  if (++ptr2 >= data + bytes) DDSOLD_ERRORMSG();
  ...
  if (ptr2 - ptr1 >= maxstr) DDSOLD_ERRORMSG();
  ...
  else DDSOLD_ERRORMSG();

  if ((image = (unsigned char*)malloc((*width)*(*height)*(*components))) == NULL) DDSOLD_ERRORMSG();
  if (data + bytes != ptr2 + (*width)*(*height)*(*components)) DDSOLD_ERRORMSG();
...
void DDSV3Old::DDS_decode (unsigned char* chunk, unsigned int size,
                        ...
                        unsigned int block)
{
  ...
  while ((cnt1 = DDS_readbits(DDS_RL)) != 0)
  {
    ...
    for (cnt2 = 0; cnt2<cnt1; cnt2++)
    {
      ...
      if ((cnt&(DDS_BLOCKSIZE - 1)) == 0) {
        if (ptr1 == NULL)
        {
          if ((ptr1 = (unsigned char*)malloc(DDS_BLOCKSIZE)) == NULL) DDSOLD_MEMERROR();
          ptr2 = ptr1;
        }
        else
        {
          if ((ptr1 = (unsigned char*)realloc(ptr1, cnt + DDS_BLOCKSIZE)) == NULL) DDSOLD_MEMERROR();
          ptr2 = &ptr1[cnt];
        }
      }
  ...
  if (ptr1 != NULL)
    if ((ptr1 = (uint8_t *)realloc(ptr1, cnt)) == NULL) DDSOLD_MEMERROR();
...
void DDSV3Old::DDS_interleave (unsigned char* data,
                            unsigned int bytes,
                            unsigned int skip,
                            unsigned int block)
{
...
void DDSV3Old::DDS_deinterleave (unsigned char* data,
                              unsigned int bytes,
                              unsigned int skip,
                              unsigned int block,
                              bool restore)
{
  ...
  if (block == 0)
  {
    if ((data2 = (unsigned char*)malloc(bytes)) == NULL) DDSOLD_MEMERROR();
  ...
  {
    if ((data2 = (unsigned char*)malloc((bytes < skip*block) ? bytes : skip*block)) == NULL) DDSOLD_MEMERROR();
...
unsigned char* DDSV3Old::readDDSfile (const char *filename, unsigned int *bytes)
{
  char DDS_ID[] = "DDS v3d\n";
  ...
  if ((chunk = readRAWfiled(file, &size)) == NULL) DDSOLD_IOERROR();
...
unsigned char* DDSV3Old::readRAWfiled (FILE *file, unsigned int *bytes)
{
  unsigned char* data;
  ...
  do
  {
    if (data == NULL)
    {
      if ((data = (unsigned char*)malloc(DDS_BLOCKSIZE)) == NULL) DDSOLD_MEMERROR();
    }
    else
      if ((data = (unsigned char*)realloc(data, cnt + DDS_BLOCKSIZE)) == NULL) DDSOLD_MEMERROR();
  ...
  if ((data = (unsigned char*)realloc(data, cnt)) == NULL) DDSOLD_MEMERROR();

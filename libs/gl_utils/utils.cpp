...
void ExitOnGLError(const char *error_message) {
  ...
  if (ErrorValue != GL_NO_ERROR) {
    ...
    memcpy(&display_message[message_length], APPEND_DETAIL_STRING,
           APPEND_LENGTH);
...
GLuint LoadShader(const char *file_name, GLenum shader_type) {
  ...
  if (((err = fopen_s(&file, file_name, "rb")) != 0) &&
      0 == fseek(file, 0, SEEK_END) && -1 != (file_size = ftell(file))) {
    rewind(file);
        ...
        if (0 != (shader_id = glCreateShader(shader_type))) {
          ...
          if (ErrorValue != GL_NO_ERROR) {
            ...
            char *display_message =
                (char *)malloc(message_length + APPEND_LENGTH);

            memcpy(display_message, "Could not compile a shader",
                   message_length);
            memcpy(&display_message[message_length], APPEND_DETAIL_STRING,
                   APPEND_LENGTH);
...
char *TextFileRead(const char *file_name) {
  ...
  if (in) {
    ...
    // thrown by std::wstring_convert.to_bytes() for bad conversions
      ...
      printf("\nFile \"%s\" could not be converted from UTF-8 to wstring (wide "
             "string)",
             file_name);
      getchar();
      exit(1);
    ...
    std::use_facet<std::ctype<wchar_t>>(std::locale("en_US.UTF-8"))
        .narrow(wstr.data(), wstr.data() + wstr.size(), ' ', result);
    result[wstr.size()] = '\0';
...
void Vertex3f(glm::vec3 v) { glVertex3f(v.x, v.y, v.z); }

void Rotatef(float angle, glm::vec3 v) { glRotatef(angle, v.x, v.y, v.z); }

void Color3f(glm::vec3 color) { glColor3f(color.x, color.y, color.z); }

void Translatef(glm::vec3 t) { glTranslatef(t.x, t.y, t.z); }
} // namespace gl
